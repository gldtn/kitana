...
Keep Quickshell color role shape in `Config/Colors.qml` and `themes/static/*.jsonc`. Static themes and `kitana-matugen` should write the same strict `Theme/current.json` schema instead of generating `Colors.qml`. Reusable QML should use semantic `Colors` roles, not native theme source names.
